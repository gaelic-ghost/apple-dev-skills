...
- Recommend `apple-xcode-workflow` when the user needs Apple or Swift execution, diagnostics, build or test work, toolchain help, or mutation decisions inside an existing project.
...
- Recommend `apple-xcode-workflow` directly when the user’s task shifts from docs exploration to Apple or Swift execution work.
